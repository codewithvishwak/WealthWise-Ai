...
WealthWise AI follows a **client-side architecture** with no backend dependencies, making it lightweight and instantly deployable.
...
│    ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│    │  Auth.js     │  │ Dashboard.js │  │  Charts.js   │     │
│    │  (Sessions)  │  │  (CRUD Ops)  │  │ (Rendering)  │     │
│    └──────────────┘  └──────────────┘  └──────────────┘     │
└─────────────────────────────────────────────────────────────┘
                              │
┌────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │              ml_classifier.js                       │   │
│  │  • ExpenseClassifier (Priority Detection)           │   │
│  │  • RecommendationEngine (Smart Suggestions)         │   │
│  │  • GamificationEngine (XP & Challenges)             │   │
│  └─────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────┘
                              │
┌────────────────────────────────────────────────────────────┐
│                    DATA PERSISTENCE                        │
│  ┌──────────────────────┐  ┌────────────────────────────┐  │
│  │   LocalStorage       │  │   Firebase (Optional Sync) │  │
│  │   (Primary Source)   │  │   (Cloud Backup)           │  │
│  └──────────────────────┘  └────────────────────────────┘  │
└────────────────────────────────────────────────────────────┘
```
...
The expense priority classifier uses a **rule-based ML approach** with weighted scoring:

```
Input: Expense {description, amount, category, date}
                    │
                    ▼
        ┌───────────────────────┐
        │  Feature Extraction   │
...
|----------|----------|--------|
| Urgent | emergency, hospital, medicine | +5 |
| Essential | rent, electricity, water, food | +4 |
| Regular | transport, groceries, phone | +3 |
| Discretionary | entertainment, shopping, dining | +1 |

### Data Flow

1. **User Action** → Validates input → Stores in LocalStorage
2. **LocalStorage** → Primary data source (offline-first)
3. **Firebase Sync** → Background sync for cloud backup
4. **UI Update** → Real-time dashboard refresh with Chart.js

### Gamification System

```
User Action ──► XP Calculation ──► Level Update ──► Challenge Progress
     │               │                   │                  │
     │               │                   │                  ▼
     │               │                   │          ┌──────────────┐
     │               │                   │          │ Achievements │
     ...
     ▼               ▼                   ▼
┌─────────┐   ┌───────────┐      ┌─────────────┐
│ +5 XP   │   │ Save to   │      │ Update UI   │
│ Expense │   │ Storage   │      │ Progress Bar│
└─────────┘   └───────────┘      └─────────────┘
```
